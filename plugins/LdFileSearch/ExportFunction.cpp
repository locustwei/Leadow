...
WJS_API WJINDEXFILE WJCreateIndexFile(WJSVOLUME volume, const TCHAR* name, WJS_CALL_BACK* callback, PVOID Param)
{
	if (g_Lib)
	{
...
WJS_API WJ_ERROR_CODE WJS_CALL WJSSearchIndexFile(WJINDEXFILE indexfile, WJS_CALL_BACK* callback, PVOID Param)
{
	if (g_Lib)
	{
		CExportSearchHandlerImpl* impl = new CExportSearchHandlerImpl();
		impl->param = Param;
		impl->callback = callback;
		return g_Lib->SearchIndexFile(indexfile, impl);
	}
	else
		return WJERROR_NOT_EXISTS;
}

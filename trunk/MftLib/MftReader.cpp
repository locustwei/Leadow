...
namespace LeadowLib
{
	...
	BOOL CMftReader::ReadSector(UINT64 sector, DWORD count, PVOID buffer)
	{
		...
		return ::ReadFile(m_Handle, buffer, count * m_Volume->GetBpbData()->BytesPerSector, &n, &overlap);
	}
	...
	*/
	...
	BOOL CMftReader::Callback(UINT64 ReferenceNumber, PFILE_INFO pFileInfo, UINT_PTR Param)
	{
		if (m_Holder)
